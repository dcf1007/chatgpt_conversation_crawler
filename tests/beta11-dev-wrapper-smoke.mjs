...
assert.doesNotMatch(server, /mhtml-dev-hook|manual-inspection|CHATGPT_CRAWLER_MANUAL_INSPECTION/, 'normal beta11 server must stay diagnostic-free');
...
assert.doesNotMatch(crawlerCore, /manual-inspection|mhtml/i, 'beta11 crawler core must remain unchanged by diagnostics');

assert.match(recorder, /diagnosticId: diagnosticState\.id/);
assert.doesNotMatch(recorder, /\bjobId\b/);

console.log('beta11-dev wrapper isolation smoke test passed');
