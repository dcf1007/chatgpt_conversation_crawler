...
assert.equal(packageJson.version, '1.7.0-beta1-dev');
...
for (const required of [
  ...
  assert.ok(fs.existsSync(path.join(root, required)), `required v1.7 beta1 file missing: ${required}`);
}
...
console.log('v1.7 beta1 package contract smoke test passed');
