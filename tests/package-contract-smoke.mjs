...
assert.equal(packageJson.version, '1.7.1-beta2-dev');
...
for (const required of [
  ...
  assert.ok(fs.existsSync(path.join(root, required)), `required v1.7.1 beta2 dev file missing: ${required}`);
}

console.log('v1.7.1 beta2 dev package contract smoke test passed');
