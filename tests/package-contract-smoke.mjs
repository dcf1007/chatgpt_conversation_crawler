...
assert.equal(packageJson.version, '1.7.1-beta3-dev2.1');
assert.equal(packageJson.scripts?.start, 'node server-dev.mjs');

for (const required of [
  'server.mjs',
  'server-dev.mjs',
  'V1_7_1_BETA3_DEV2_1.md',
  'README.v1.7.1-beta3-dev2.1.txt',
  ...
  assert.ok(fs.existsSync(path.join(root, required)), `required v1.7.1 beta3-dev2.1 file missing: ${required}`);
}

console.log('v1.7.1 beta3-dev2.1 package contract smoke test passed');
