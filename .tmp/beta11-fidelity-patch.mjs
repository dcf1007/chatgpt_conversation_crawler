...
const imagesOld = String.raw`  for (const record of records) {
...
`;
const imagesNew = String.raw`  let referenced = 0;
  let unreferenced = 0;
...
fs.writeFileSync('tests/snapshot-media-control-smoke.mjs', `import assert from 'node:assert/strict';
import fs from 'node:fs';
...
fs.writeFileSync('tests/main-image-finalization-smoke.mjs', `import assert from 'node:assert/strict';
...
fs.writeFileSync('tests/session-state-refresh-smoke.mjs', `import assert from 'node:assert/strict';
import fs from 'node:fs';
