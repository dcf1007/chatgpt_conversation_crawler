...
/**
 * Endpoint stability intentionally excludes mountedFirst/mountedLast and the
 * raw mounted-control census. Those describe ChatGPT's transient virtualizer
 * window, not capture progress. Retained richness, traversal position, and
 * actual expansion counters are the durable signals that can justify moving on.
 */
...
export async function scan(page, direction, pass, onProgress, shouldCancel, maxSteps = SCAN_MAX_STEPS) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    const atEnd = direction === 'down'
      ? metrics.top >= maximumTop - 4
      : metrics.top <= 4;
    const stats = await page.evaluate(() => window.__archiveCrawler.stats());
    ...
    const positionPercent = maximumTop <= 0
      ? 100
      : Math.max(0, Math.min(100, (metrics.top / maximumTop) * 100));
...
export async function verifyOldestMessages(page, onProgress, shouldCancel) {
  ...
  for (let check = 0; check < OLDEST_MAX_CHECKS; check++) {
    ...
    if (!atTop) {
      ...
      // A short excursion forces ChatGPT to expose any lazily prepended edge
      // generation without making virtualizer membership part of convergence.
      const maximumTop = Math.max(0, metrics.height - metrics.client);
  ...
  const result = {
    ...
    quietChecks,
    checks,
    requiredQuietChecks: OLDEST_REQUIRED_QUIET_CHECKS,
    maxChecks: OLDEST_MAX_CHECKS
  };
  await page.evaluate(value => window.__archiveCrawler.markOldestVerification(value), result);

  await report(page, onProgress, {
    phase: converged
      ? 'Oldest-message verification complete'
      : 'Oldest-message verification safety limit reached',
    detail: converged
...
function rewriteReconciliationProgress(onProgress, round, direction) {
  ...
  return progress => {
    ...
    return onProgress({
      ...
      detail: 'Rich retained turns still contain recognized collapsed disclosures. Revisiting the conversation with turn-scoped convergence so remounted roots can expose descendants without waiting on unrelated virtualizer churn.',
...
/**
 * Reconciliation remains a corpus-level safety net because a rich turn may
 * remount collapsed after an earlier successful activation. beta8 does not,
 * however, demand two identical full traversals to prove that historical
 * retained flags are inert. One complete pass with no retained-fingerprint
 * improvement is sufficient to stop; a second pass is run only when the first
 * pass actually changed the unresolved retained corpus.
 */
export async function reconcileRetainedDisclosures(page, onProgress, shouldCancel) {
  ...
  for (let round = 1; round <= RECONCILIATION_MAX_PASSES; round++) {
    ...
    await onProgress?.({
      ...stats,
      phase: 'Reconciling retained disclosures',
      detail: 'The retained corpus still contains disclosures captured collapsed. A turn-scoped reconciliation traversal will revisit them; an unchanged retained fingerprint now stops immediately.',
    ...
    // scan() already exhausts mounted disclosures at every scroll position and
    // performs a full retained capture. beta7's additional post-scan
    // expand/stabilize sequence duplicated that work and is intentionally gone.
  ...
  await report(page, onProgress, {
    phase: result.converged
      ? 'Retained-disclosure reconciliation complete'
      : 'Retained-disclosure reconciliation stopped',
    detail: result.converged
      ? `All retained recognized disclosures converged after ${rounds} reconciliation pass(es).`
      : `Reconciliation stopped after ${rounds} pass(es) with ${result.unresolvedTurns} retained turn(s) and ${result.unresolvedDisclosures} disclosure(s) still flagged. Their richest captured versions are preserved; the development manual comparison remains available for verification.`,
...
export async function crawlAutomaticConversation(page, { onProgress, shouldCancel } = {}) {
  ...
  await onProgress?.({
    ...
    detail: oldest.converged
      ...
      : 'Traversal reached the oldest-edge safety limit; retained-corpus reconciliation is complete and remaining mounted disclosures are being checked before the final snapshot.',
