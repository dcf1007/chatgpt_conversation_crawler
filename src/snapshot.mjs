...
export async function buildSnapshot(page, sourceUrl, { preview = false, embedImages: shouldEmbed = !preview, archiveState = null } = {}) {
  ...
  const snapshot = await page.evaluate(({ sourceUrl, archivedAt, preview, tokenizeImages, tokenPrefix, archiveOverride }) => {
    ...
    const integrityDiagnostic = !preview && integrityWarnings.length
      ? `<details class="archive-diagnostics archive-warning" open><summary>${integrityWarnings.length} archive integrity warning(s)</summary><ul>${integrityWarnings.map(item => `<li>${escLocal(item.message || item.code || 'Archive integrity warning')}</li>`).join('')}</ul><p>All non-duplicated content observed from competing hydration generations was retained. A warning means the crawler could not prove one or more convergence conditions before its safety limits.</p></details>`
      : '';
